...
parser.add_argument('--outlier_interval', metavar='', type=float, default=1, help='interval between each consecutive outlier threshold')
...
interval = args.outlier_interval
...
class RHRAD_online:
    ...
    # Positive Anomalies -----------------------------------------------------------------
        ...
        """
    def positive_anomalies(self, data, k):
        ...
        positive_anomalies.to_csv("_" + str(round(lst[k], 1)) + "_" + myphd_id_anomalies, header=True) 
    ...
    def create_alerts(self, anomalies, data, fitbit_oldProtocol_hr, k):
        ...
        daily_alerts.to_csv("_" + str(round(lst[k], 1)) + "_" + myphd_id_alerts,  na_rep='NA', header=True) 
    ...
    def visualize(self, results, positive_anomalies, test_alerts, symptom_date, diagnosis_date, k):
        """
        ...
        try:
            ...
            with plt.style.context('fivethirtyeight'):
                ...
                figure = fig.savefig("_" + str(round(lst[k], 1)) + "_" + myphd_id_figure1, bbox_inches='tight')                             
                return figure
    ...
    def intervalgenerator(self, start, stop, interval):
        lst = []
        next = start
        while next <= stop:
            lst.append(next)
    ...
    def numAlerts(self, alerts, k):
        numRed = 0
        numGreen = 0
        numYellow = 0
        alerts = alerts.dropna()
        for i in range(len(alerts)):
            alert = alerts.iloc[i, 1]
            if alert == "GREEN":
                numGreen +=1
            elif alert == "RED":
                numRed +=1
        ...
        alerts_df.to_csv("_" + str(round(lst[k], 1)) + "_" + "num_alerts.csv")
        return alerts_df

    def createoutliergraph(self):
        labels = intervalgenerator(start, stop, interval)
...
model = RHRAD_online()
lst = model.intervalgenerator(start, stop, interval)
...
for k in range(len(lst)):
    ...
    data_train = []
    data_test = []
    model.online_anomaly_detection(data_seasnCorec, baseline_window, sliding_window, lst[k])
